...
class DM2FNet_woPhy_loss_weight_average(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_loss_weight_average, self).__init__()
        self.num_features = num_features
...
class DM2FNet_woPhy_loss_weight_x0(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_loss_weight_x0, self).__init__()
        self.num_features = num_features
...
class DM2FNet_woPhy_inference_weight_x0(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_inference_weight_x0, self).__init__()
        self.num_features = num_features
    ...
    def forward(self, x0):
        ...
        x_fusion = torch.cat((torch.sum(F.softmax(attention_fusion[:, : 4, :, :], 1) * torch.stack(
            (x_p0[:, 0, :, :] * 1.3,
             ...
             x_p2[:, 0, :, :] * 0.9, 
             x_p3[:, 0, :, :] * 0.9), 1), 1, True),
                              torch.sum(F.softmax(attention_fusion[:, 4: 8, :, :], 1) * torch.stack((x_p0[:, 1, :, :] * 1.3,
                                                                                                     x_p1[:, 1, :, :] * 0.9,
                                                                                                     x_p2[:, 1, :, :] * 0.9,
                                                                                                     x_p3[:, 1, :, :] * 0.9),
                                                                                                    1), 1, True),
                              torch.sum(F.softmax(attention_fusion[:, 8:, :, :], 1) * torch.stack((x_p0[:, 2, :, :] * 1.3,
                                                                                                   x_p1[:, 2, :, :] * 0.9,
                                                                                                   x_p2[:, 2, :, :] * 0.9,
                                                                                                   x_p3[:, 2, :, :] * 0.9),
                                                                                                  1), 1, True)),
...
class DM2FNet_woPhy_pro(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(DM2FNet_woPhy_pro, self).__init__()
        self.num_features = num_features
        ...
        self.fuse3_attention_1 = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=1), nn.Sigmoid()
        )
        self.fuse2_attention_1 = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=1), nn.Sigmoid()
        )
        self.fuse1_attention_1 = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=1), nn.Sigmoid()
        )
        self.fuse0_attention_1 = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=1), nn.Sigmoid()
        )
    ...
    def forward(self, x0):
        ...
        down4 = F.upsample(down4, size=down3.size()[2:], mode='bilinear')
        fuse3_attention = self.fuse3_attention(torch.cat((down4, down3), 1))
        # print("hidden shape: ", fuse3_attention.shape)
        fuse3_attention = fuse3_attention + self.fuse3_attention_1(fuse3_attention)
        f = down4 + self.fuse3(torch.cat((down4, fuse3_attention * down3), 1))

        f = F.upsample(f, size=down2.size()[2:], mode='bilinear')
        fuse2_attention = self.fuse2_attention(torch.cat((f, down2), 1))
        fuse2_attention = fuse2_attention + self.fuse2_attention_1(fuse2_attention)
        f = f + self.fuse2(torch.cat((f, fuse2_attention * down2), 1))

        f = F.upsample(f, size=down1.size()[2:], mode='bilinear')
        fuse1_attention = self.fuse1_attention(torch.cat((f, down1), 1))
        fuse1_attention = fuse1_attention + self.fuse1_attention_1(fuse1_attention)
        f = f + self.fuse1(torch.cat((f, fuse1_attention * down1), 1))

        f = F.upsample(f, size=down0.size()[2:], mode='bilinear')
        fuse0_attention = self.fuse0_attention(torch.cat((f, down0), 1))
        fuse0_attention = fuse0_attention + self.fuse0_attention_1(fuse0_attention)
        f = f + self.fuse0(torch.cat((f, fuse0_attention * down0), 1))
...
class DM2FNet_woPhy_Unet(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        self.fuse3_attention_1 = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1, stride=2), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1, stride=2), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.ConvTranspose2d(in_channels=num_features, out_channels=num_features, kernel_size=3, stride=2, padding=1, output_padding=1), nn.SELU(),
            nn.ConvTranspose2d(in_channels=num_features, out_channels=num_features, kernel_size=3, stride=2, padding=1, output_padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=1), nn.Sigmoid()
        )
        self.fuse2_attention_1 = nn.Sequential(
    ...
    def forward(self, x0):
        ...
        down4 = F.upsample(down4, size=down3.size()[2:], mode='bilinear')
        fuse3_attention = self.fuse3_attention(torch.cat((down4, down3), 1))
        # print("hidden shape 1: ", fuse3_attention.shape)
        fuse3_attention = fuse3_attention + self.fuse3_attention_1(fuse3_attention)
        # print("hidden shape 2: ", fuse3_attention.shape)
        f = down4 + self.fuse3(torch.cat((down4, fuse3_attention * down3), 1))
